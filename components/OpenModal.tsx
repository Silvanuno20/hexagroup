...
import { X, Database, Loader2, ChevronRight, ArrowLeft, Clock, Search, LayoutGrid, FileUp } from 'lucide-react';

const translations = {
  pt: {
    titleMenu: "Abrir Projeto",
    titleLibrary: "Biblioteca de Rodas",
    titleRecent: "Recentes",
    cloudLabel: "Nuvem Supabase",
    cloudSub: "Ver salvos online",
    localLabel: "Ficheiro Local",
    localSub: "Carregar do dispositivo",
    search: "Pesquisar...",
    syncing: "Sincronizando...",
    viewAll: "Ver todos os projetos em grelha",
    noWheels: "Nenhuma roda encontrada."
  },
  en: {
    titleMenu: "Open Project",
    titleLibrary: "Wheel Library",
    titleRecent: "Recent",
    cloudLabel: "Supabase Cloud",
    cloudSub: "View online saves",
    localLabel: "Local File",
    localSub: "Upload from device",
    search: "Search...",
    syncing: "Syncing...",
    viewAll: "View all projects in grid",
    noWheels: "No wheels found."
  }
};
      ...
      <div className={`bg-[#0a0a0a] w-full rounded-[32px] border border-white/10 flex flex-col transition-all duration-300 shadow-2xl overflow-hidden ${showAll ? 'max-w-4xl h-[80vh]' : 'max-w-md h-[550px]'}`}>
        ...
        <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
          <div className="flex items-center gap-3">
            {view !== 'menu' && (
              <button onClick={() => { setView('menu'); setShowAll(false); }} className="p-2 hover:bg-white/5 rounded-full transition-colors text-white">
                <ArrowLeft size={20} />
              </button>
            )}
            <h2 className="font-black uppercase tracking-tighter text-lg text-white">
              {view === 'menu' ? t.titleMenu : (showAll ? t.titleLibrary : t.titleRecent)}
            </h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
          {view === 'menu' ? (
            <div className="grid gap-4">
              {/* Opção Nuvem */}
              <button onClick={() => setView('cloud')} className="w-full p-6 bg-white/5 rounded-2xl flex items-center justify-between hover:bg-white/10 transition-all border border-white/5 group text-white">
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-purple-500/20 rounded-xl group-hover:bg-purple-500/30 transition-colors">
                    <Database className="text-purple-500" size={24} />
                  </div>
                  <div className="text-left">
                    <span className="block font-bold">{t.cloudLabel}</span>
                    <span className="block text-[10px] text-gray-500 uppercase font-black tracking-widest mt-1">{t.cloudSub}</span>
                  </div>
                </div>
              ...
              <button onClick={() => fileInputRef.current?.click()} className="w-full p-6 bg-white/5 rounded-2xl flex items-center justify-between hover:bg-white/10 transition-all border border-white/5 group text-white">
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-blue-500/20 rounded-xl group-hover:bg-blue-500/30 transition-colors">
                    <FileUp className="text-blue-500" size={24} />
                  </div>
                  <div className="text-left">
                    <span className="block font-bold">{t.localLabel}</span>
                    <span className="block text-[10px] text-gray-500 uppercase font-black tracking-widest mt-1">{t.localSub}</span>
                  </div>
                </div>
                <ChevronRight size={18} className="text-gray-600" />
              </button>
            ...
            <div className="space-y-6">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-2xl px-11 py-3 text-sm text-white focus:ring-2 focus:ring-purple-500/50 outline-none transition-all"
                />
              </div>
              ...
              {isLoading ? (
                <div className="flex flex-col items-center justify-center py-20 text-gray-500">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[3px]">{t.syncing}</span>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className={showAll || searchTerm !== "" ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" : "space-y-2"}>
                    ...
                    {filtered.length === 0 && (
                       <div className="col-span-full text-center py-10 text-gray-600 text-xs font-bold uppercase tracking-widest">{t.noWheels}</div>
                    )}
                  </div>
